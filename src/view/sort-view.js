...
import { SortType, DISABLED_SORT_TYPES, SortTypeDescriptions } from '../consts';

const createSortTemplate = (currentSortType) => {
  const sortTypes = Object.values(SortType);
  const sortsTemplate = sortTypes.map((sortType) =>
    `<div class="trip-sort__item  trip-sort__item--${sortType}">
      <input ${currentSortType === sortType ? 'checked' : ''} id="sort-${sortType}" data-sort-type=${sortType} class="trip-sort__input  visually-hidden" type="radio" name="trip-sort" value="sort-${sortType}" ${DISABLED_SORT_TYPES.includes(sortType) ? 'disabled' : ''}>
